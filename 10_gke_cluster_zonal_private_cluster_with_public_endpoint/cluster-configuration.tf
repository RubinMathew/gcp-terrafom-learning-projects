...
resource "google_container_cluster" "gke-private-cluster" {
  ...
  subnetwork         = google_compute_subnetwork.subnets-vpc[1].name #subnet should be available in the location specified above is subnet should be available at us-central1
  ...
  node_pool {
    name       = "e2-micro-pool"
    node_count = 2   #this is a regional cluster, yu will get 2 node totally , 
    #if this cluster is a regional cluster , then node_count=2 means , there  6 node is generated(2 for us-central1-a, us-central1-b, us-central1-b. us-central1-d zone hos no kubernetes support)
    node_config {
